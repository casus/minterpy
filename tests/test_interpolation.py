...
import numpy as np
import pytest
from conftest import SpatialDimension,PolyDegree,LpDegree,MultiIndices,NrPoints,assert_polynomial_almost_equal, assert_call,assert_multi_index_equal, assert_grid_equal,assert_interpolant_almost_equal,build_random_newton_polynom,build_rnd_points
from numpy.testing import assert_,assert_almost_equal

import minterpy as mp
from minterpy import Interpolator,Interpolant,interpolate
...
def test_init_interpolator(SpatialDimension,PolyDegree,LpDegree):
    ...
    groundtruth_multi_index = mp.MultiIndexSet.from_degree(SpatialDimension,PolyDegree,LpDegree)
...
def test_init_interpolant(SpatialDimension,PolyDegree,LpDegree):
    assert_call(Interpolant, lambda x : x[:,0] ,Interpolator(SpatialDimension,PolyDegree,LpDegree))
    assert_call(Interpolant.from_degree, lambda x : x[:,0] ,SpatialDimension,PolyDegree,LpDegree)
    interpolant_default = Interpolant(lambda x : x[:,0] ,Interpolator(SpatialDimension,PolyDegree,LpDegree))
    interpolant_from_degree = Interpolant.from_degree(lambda x : x[:,0] ,SpatialDimension,PolyDegree,LpDegree)
...
def test_interpolator(SpatialDimension,PolyDegree,LpDegree):
    groundtruth_poly = build_random_newton_polynom(SpatialDimension,PolyDegree,LpDegree)
    interpolator = Interpolator(SpatialDimension,PolyDegree,LpDegree)
...
def test_interpolant(NrPoints,SpatialDimension,PolyDegree,LpDegree):
    ...
    interpolant = Interpolant.from_degree(groundtruth_poly,SpatialDimension,PolyDegree,LpDegree)
